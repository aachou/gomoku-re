...
│   └── ui.py             # Tkinter 桌面图形界面（悔棋/步数历史/调速/计时器/主题/热键提示/存档读档/确认弹窗/对局统计）
...
- 坐标格式支持 `A1` 或 `1 1`。
- 桌面 UI 支持：悔棋（Ctrl+Z）、步数历史、双方计时、AI 速度调节、主题切换（默认/森林/暖阳）、热键提示（按 `?` 查看）、落子闪烁动画、存档读档（Ctrl+S/Ctrl+L）、退出确认、对局统计弹窗。
