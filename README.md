...
- 坐标格式支持 `A1` 或 `1 1`。
- 桌面 UI 支持：悔棋（Ctrl+Z）、步数历史、双方计时、AI 速度调节、主题切换（默认/森林/暖阳）、热键提示（按 `?` 查看）、落子闪烁动画、存档读档（Ctrl+S/Ctrl+L）、退出确认、对局统计弹窗、跨 session 统计持久化、重新开始、全屏/窗口切换（默认 1200×800，最小 900×640，窗口居中）、主菜单滚轮滚动。
